...
generated request id, a best-effort client hint, and the username that token belongs
to (looked up in config.VAULT_MCP_TOKENS). The audit module reads them when building
a record. It lives in its own module so
auth.py (the writer) and audit.py (the reader) share one ContextVar without importing
each other.
"""
...
def set_request_context(
    principal: str | None, request_id: str | None, client: str | None,
    username: str | None = None,
) -> Token:
    """Bind the audit context for the current request. Returns a reset token."""
    return _request_context.set(
        {"principal": principal, "request_id": request_id, "client": client, "username": username}
    )
